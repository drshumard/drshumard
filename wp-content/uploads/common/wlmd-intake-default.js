/*
    web/_FRAMEWORK_WP/wp-content/uploads/common/wlmd-intake-func-v4.8.js
    2/1/25 - confirmed 5/29/25
*/

// goToCheckout - user function to push to checkout after product selection.
...
function tracking_finished() {
    if(window.trackingInfo['unid']) {
        //console.log('YES tracking_finished');
        window.trackingInfo.finished = 1;
        return true;
    }
    //console.log('NO not tracking_finished');
    return false;
}
...
function handleJotformReadyCustomFunction() {
    if(typeof window.custom_form_ready === "function") {
        ...
        elog("handleJotformReadyCustomFunction - No custom_form_ready found");
    }
}
...
function uniqueIdFieldOfIframe(form_element) {
    return fieldOfIFrame("uniqueId",form_element);
}
function callerFieldOfIframe(form_element) {
    var ret = fieldOfIFrame("__caller",form_element);
    if(ret) {
        return ret;
    }
    ret = fieldOfIFrame("caller_page",form_element);
    return ret;
}
function forcedUniqueIdFieldOfIframe(form_element) {
    return fieldOfIFrame("forced_unid",form_element);
}
function trackingFieldOfIframe(form_element) {
    return fieldOfIFrame("tracking_unid",form_element);
}
function trackingArgsFieldOfIframe(form_element) {
    return fieldOfIFrame("__tracking_args",form_element);
}

function ifrFormObject(ifr) {
    //console.log('ifrFormObject:');
    //console.log(ifr);
    var ifrDoc;
    if (ifr && ifr.formID) {
        //alert(typeof ifr);
        ...
        if(ifrDoc) {
            return ifrDoc.forms[0]??null;
        } else {
            return null;
        }
    }
}
function myContentDoc() {
    if(!window.jotform_id) {
        elog("myContentDoc - no jotform_id yet. return false");
        return false;
    }
    console.log("myContentDoc: " + window.jotform_id);
    ifr = document.getElementById(window.jotform_id);
    var ifrDoc;
    if (ifr.formID) {
        //alert(typeof ifr);
        ifrDoc = ifr;
        ...
        ifrDoc = ifr.contentDocument || ifr.document;
        if(ifrDoc) {
            return ifrDoc;
        } else {
            return null;
...
function jotformContentDocument() {
    const ifr = document.getElementById(window.jotform_id);
    if (ifr && ifr.formID) { // form is inside main document.
        return document;
    } else {
...
function fieldOfIFrame(name_ends, ifr) {
    //var ifr = window.IFR;
    var theForm = ifrFormObject(ifr);
    if(!theForm) {
        elog('ERROR! in trackingFieldOfIframe, no ifrFormObject',1);
        return null;
    }
...
function setupIntakeTracking() {
    ...
    if(window.trackingInfo['codeset']) { // this really should be true.
        ...
        try {
            ...
            // uniqueIdFieldOfIframe(ifr).change();
            //console.warn("using uniqueIdFieldOfIframe, trackingFieldOfIframe not found.");
        } catch(e) {
            console.warn("uniqueIdFieldOfIframe cant be set:");
            console.warn(e);
        }
        try { callerFieldOfIframe(ifr).value = urlNoHash(); } catch(e) { console.warn('no caller field in form.'); }
    ...
    else { elog("---CODESET NOT TRUE--"); }

    var urlParams = new URLSearchParams(window.location.search); //get all parameters
    var tuid = urlParams.get("uniqueId");
    if(tuid) {
        elog('setupIntakeTracking - set order-gen-uid:' + tuid, 1);
    ...
    if(window.forced_uniqueId) {
        elog("setupIntakeTracking - FORCE unid to " + window.forced_uniqueId,1);
        addToRecordQueue('forced_uniqueId', window.forced_uniqueId);

        tmpfield = forcedUniqueIdFieldOfIframe(ifr);
        if(tmpfield) {
            elog('found forcedField');
            tmpfield.value = window.forced_uniqueId;
        } else {
            elog('setupIntakeTracking - DID NOT found forcedField');
        }
        tmpfield = uniqueIdFieldOfIframe(ifr);
        if(tmpfield) {
            tmpfield.value = window.forced_uniqueId;
            elog('setupIntakeTracking - found unid Field and set to jotform uniqueId field to ' + window.forced_uniqueId);
        } else {
            elog('setupIntakeTracking - DID NOT found unid Field in jotform');
        }
    ...
    //try { ctrlScriptFieldOfIframe(window.IFR).value = urlNoHash(); } catch(e) { console.warn("no ctrlScript field"); }
    elog("setupIntakeTracking finished");
}

function addPagePreset(key,value) {
    window.page_presets[key] = value;
    setJotformField(key, value);
}
...
function setTriggerJotformField(name, value) {
    var field = fieldOfIFrame(name, window.IFR);
    if(field) {
        field.value = value;
...
function triggerJotformField(name) {
    var field = fieldOfIFrame(name, window.IFR);
    if(field) {
        //field.value = value;
        const changeEvent = new Event('change');
...
function makeListenerField(name_type) { // you can just pass the raw name.
    ...
    if(fieldName.substring(0,7) === '__timer') {
        //console.warn('skip timer field ' + fieldName);
        return false;
    }
    if(fieldName.substring(0,6) === '__next') {
        //console.warn('skip next field ' + fieldName);
        return false;
    }
    if(myArray.length === 1) {
        myArray[1] = "change";
    ...
    const FIELDS = jotformContentDocument().querySelectorAll('[name="' + origName + '"]');

    if(!FIELDS || !FIELDS[0]) {
        console.warn('makeListener, no field for ' + name_type);
    }
...
function postQueuedData() {
    ...
    for(var key in tmp) {
        ...
        let myvalue = "";
        if(typeof(window.recordQueue[key])=='object') {
           //elog("Alreay object: " + key + " = " + JSON.stringify(window.recordQueue[key]) );
            myvalue =  window.recordQueue[key];
        } else {
           //console.warn("Make array from " + key + ":" +window.recordQueue[key]);
           myvalue = { "name": key, "q_name": key, "value": window.recordQueue[key]};
        }
        records.push(myvalue);
...
function trackButtonClick(jsForm, mystep, totalsteps, action) {
    var anon = function () {
        ...
        var numOfErrors = jotformContentDocument().getElementsByClassName('form-error-message').length;
        var params = {
            //'part': window.mystage,
            'my_step': mystep,
        ...
        if (numOfErrors !== 0) {
            ...
            if(action === "submit") params['action'] = "submit";
...
function jumpToTop() {
    console.log('called jumptotop');
    let v=0;
    try {
        let el = document.getElementById(window.jotform_id);
        let rect={};
        if (el &&
            (rect = getOffset(el))
        ) {
        ...
        console.warn('error getting top of form of: ', window.jotform_id);
    }
...
function activateFormEvents(jsForm) {
    ...
    const cdoc = jotformContentDocument();

    var questions = cdoc.getElementsByTagName("form")[0].elements;
    var backButtons = cdoc.getElementsByClassName("form-pagebreak-back") //arr
    var nextButtons = cdoc.getElementsByClassName("form-pagebreak-next") //arr
    var nextCards = cdoc.getElementsByClassName("forNext");
    var submitButtons = cdoc.getElementsByClassName("form-submit-button") //arr
    ...
    if(window.directives && window.directives['site_id']) {
        ...
        for (let i = 0; i < questions.length; i++) {
            try {
                ...
                console.warn('ERROR making question ' + i, questions[i]);
...
function doPageFormOnload() {
    ...
    var ifr;
    const ifrelem = document.getElementById(iframe_id);
    if(ifrelem) {
        ifr = ifrFormObject(ifrelem);
    } else {
        ifr = null;
    }
...
function orderProcessingContext() {
...
function advanceForm(formId, data) {
    ...
    elog('advanceForm - embed_add_args = ' + window.directives['embed_add_args']);
...
async function proceedToCheckoutReturnable(LINKS,data) {
    url = calculateCheckoutUrl(LINKS,data);
    elog('calculateCheckoutUrl AFTER REPLACE url=' + url);
    data['next_step'] = 'checkout';
    data['next_url'] = url;
...
function addArgsToCheckout(url,data) {
    ...
    if(!url) {
        console.warn("ERROR No url passed to addArgsToCheckout");
        return "";
    }
...
function userSelectedProduct() {
    ...
    var tmp;
    if(tmp = urlParams.get("product")) {
        return tmp;
    }
    if(wid_key['product'] && getWID(wid_key['product'])) {
        return getWID(wid_key['product']);
    }
    if(tmp = getJotformField('sku')) {
        setWID('product',tmp);
        return tmp;
    }
    if(tmp = getJotformField('productRequested')) {
        setWID('product',tmp);
        return tmp;
    }
...
function handleFormHashControl() {
    ...
    if (operation === "step_2") {
        ...
        if(typeof window.step3Action === "function") {
            //alert("step3action wutg " + JSON.stringify(data));
            step3Action(data);
        } else {
        ...
        if(typeof window.checkoutAction === "function") {
            //alert("checkoutAction w/ " + JSON.stringify(data));
            checkoutAction(data);
        } else {
            proceedToCheckout(steps['checkout'], data);
        }
...
function callGA(params) {
    ...
    try {
        ...
        console.warn('callGA Error running dataLayer.push',e);
        return false;
    }
    return true;
}
